...
class ChatServiceTest {
    ...
    void setUp() {
        ...
        lenient().when(chatClient.prompt().messages(anyList()).call().content())
                 .thenReturn("AI reply");
        lenient().when(summaryChatClient.prompt().user(anyString()).call().content())
                 .thenReturn("SUMMARY");
    ...
    void chat_clearCommand_removesHistoryAndSignalsCleared() {
        ...
        // verify the LLM was NOT called for the /clear itself but WAS called for "bonjour" and "hi again" (2 total)
        verify(chatClient.prompt().messages(anyList()).call(), times(2)).content();
    }
    ...
    void chat_autoCompacts_whenCharThresholdExceeded() {
        ...
        // On the third call, the pre-flight check fires auto-compact before the user message is appended
