 ...
 *
 * <ul>
 *   <li>Standalone {@link DefaultCamelContext} — no Spring Boot startup overhead
 *   <li>{@link AdviceWith} to swap the timer consumer for a controllable {@code direct:} endpoint
 *   <li>Asserting Splitter EIP fan-out behaviour
 *   <li>Asserting per-item error isolation via {@code onException}
 * </ul>
 */
...
class TaskPollRouteTest {
  ...
  void setUp() throws Exception {
    ...
    AdviceWith.adviceWith(camelContext, "task-poll", a -> a.replaceFromWith("direct:tick"));
    camelContext.start();
  }
  ...
  void onException_isolatesFailingTaskFromSiblings() throws Exception {
    ...
    var t2 = task("t2");
    when(taskPoller.findDue(any(Instant.class))).thenReturn(List.of(t1, t2));
    doThrow(new RuntimeException("task t1 failed"))
        .when(taskPoller)
        .run(eq(t1), any(Instant.class));
