...
public class PromptCreateTool implements DaneelToolInterface {
  ...
  public PromptCreateTool(
      @Value("${daneel.scheduler.tasks-dir:./tasks}") String tasksDir, ObjectMapper objectMapper) {
  ...
  public List<ToolProperty> properties() {
    return List.of(
        ...
        new ToolProperty(
            "body",
            "The prompt text sent to the LLM when the task fires. " + "May use time placeholders.",
            "string",
            true));
  }
